...
# Códigos de error para envíos de anuncios [!DNL FreeWheel]

Los mensajes de error para los envíos de anuncios fallidos pueden provenir de Advertising Cloud DSP o de [!DNL FreeWheel]. Los mensajes de error se muestran en la columna [!UICONTROL API Response] del cuadro de diálogo [[!UICONTROL Freewheel Status]](freewheel-check-status.md).

## Errores internos de Advertising Cloud DSP

| Mensaje de error | Descripción | Pasos siguientes |
|--- |--- |--- |
| [!DNL Espera respuesta de estado de [!DNL FreeWheel]] | [!DNL FreeWheel] aún no ha respondido que el envío se haya realizado correctamente o que haya fallado. | Vuelva a comprobar el estado en 10 minutos. |
...
| [!DNL The deal id you input is not setup as a guaranteed feed. Please submit guaranteed deals only.] | El acuerdo presentado no se configura como un acuerdo garantizado programático. [!DNL FreeWheel] solo acepta ofertas garantizadas. | Configure el ID de acuerdo como una oferta garantizada mediante programación. La publicidad se envía automáticamente a [!DNL FreeWheel] cuando se guarda la ubicación predeterminada garantizada mediante programación al final del flujo de trabajo de ID de acuerdo. |
...
| [!DNL \[public_id=]\&lt;deal>] no existe | El id. de oferta enviado no existe en el [!DNL FreeWheel] extremo. | Póngase en contacto con su representante de [!DNL FreeWheel] para confirmar el ID de la oferta. |
| [!DNL Ad with identifier] \&lt;>nombre *de la publicidad\>  [!DNL was not found.]* | La clave de publicidad enviada no existe o no está activa en el final del Adobe. | Busque la clave de publicidad correcta y luego vuelva a enviarla. |
...
|--- |--- |--- |--- |
| 401 | No autorizado | Credenciales de acceso incorrectas, faltantes o no válidas. | Póngase en contacto con el administrador de cuentas de Adobe. |
| 403 | Prohibido | El servidor entendió la solicitud, pero se niega a autorizarla. | Póngase en contacto con el administrador de cuentas de Adobe. |
| 404 | No encontrado | El recurso solicitado no está disponible. Si no se encuentra el ID creativo en la operación de PUT, se devuelve un error 404. | Póngase en contacto con el administrador de cuentas de Adobe. |
| 405 | Método no permitido | Se ha realizado una solicitud de un recurso utilizando un método de solicitud no admitido por ese recurso (por ejemplo, utilizando GET en un método que requiere que el POST envíe los datos o utilizando un PUT en un recurso de solo lectura). | Póngase en contacto con el administrador de cuentas de Adobe. |
| 408 | Tiempo de espera de solicitud | Se agotó el tiempo de espera mientras se procesaba esta solicitud. Los tiempos de espera suelen deberse a solicitudes simultáneas de acceso exclusivo a determinados recursos. | Vuelva a enviar la solicitud cuando reciba este estado. Si el problema persiste, póngase en contacto con el administrador de cuentas de Adobe. |
| 422 | Entidad no procesable | Recurso no válido. Este error ocurre cuando el cuerpo de la solicitud no es válido o el recurso creado/actualizado no es válido (por ejemplo, si no se encontró el ID de acuerdo). Consulte [Errores de API 422 de FreeWheel](#freewheel-422-errors) para obtener más información. | Póngase en contacto con el administrador de cuentas de Adobe. |
| 500 | Error interno del servidor | Error del sistema de API. | Póngase en contacto con el administrador de cuentas de Adobe. |
...
>* [Comprobar el estado de las publicidades  [!DNL FreeWheel] para los acuerdos garantizados programáticos](/help/dsp/inventory/freewheel-check-status.md)
